...
## Investigate: `make doc` fails on `plan/py2many-adoption`, but only for `--workspace`

**Not caused by the differential change.** Recorded here because it blocks `make check`, and
`make check` is what CI runs.

### What happens

```bash
RUSTDOCFLAGS="-D warnings" cargo doc --workspace --no-deps --lib   # 14 errors
```

`compylr-core`, `compylr-backend-rust`, and `compylr-frontend-python` fail to compile under
rustdoc against a `compylr_ir` that appears to predate behavior profiles:

```
error[E0432]: unresolved imports `compylr_ir::Axis`, `compylr_ir::Behavior`,
              `compylr_ir::LanguageBehavior`, `compylr_ir::Stance`
error[E0559]: variant `compylr_ir::BinOp::Add` has no field named `checked`
error[E0407]: method `behavior` is not a member of trait `Backend`
```

### What rules things out

- `cargo build --workspace --lib` **succeeds** on the same source.
- `cargo test --workspace` and `cargo clippy --workspace --all-targets -- -D warnings` both pass.
- `cargo doc -p compylr-ir --no-deps --lib` succeeds. So does `-p compylr-core`. **Only
  `--workspace` fails.**
- `cargo clean --doc` does not change it, so it is not a stale artifact.
- One `compylr-ir` in `cargo metadata`, at the expected path. No `[patch]`, no `.cargo/config.toml`,
  no `cfg(doc)` in `compylr-ir`, no feature gates on the re-exports, rustc and rustdoc both 1.98.0.
- The differential change touches **no** `crates/*/src/` file and no `Cargo.toml` — only files under
  `crates/compylr-host-python/tests/`, which `--lib` does not build.

### Where to start

The reported item shapes match `compylr-ir` *before* `add-behavior-profiles`, which suggests
rustdoc is resolving an older rmeta for it under workspace-wide feature unification. Worth checking
whether `crates/compylr-host-python`'s dev-dependencies — which name the same crates as its
dependencies — cause cargo to build two variants of `compylr-ir` when the whole workspace is
documented at once. `cargo doc --workspace -v` will name the actual `--extern` paths.
