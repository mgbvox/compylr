...
- [x] 1.1 Write tests that two subscript nodes declaring different index origins, and two length
      nodes declaring different text units, are distinguishable, survive serialization, and produce
      different fingerprints
- [x] 1.2 Add `IndexOrigin::{FromEitherEnd, FromStart}` and `TextUnits::{CodePoints, Utf8Bytes,
...
- [x] 1.3 Split `Expr::Len` out of the `Neg | ToFloat | Not` match arms it currently shares in
      `rust.rs` and `folding.rs`, and bump `ARTIFACT_VERSION` to 3
- [x] 1.4 Record in the IR's own documentation which container behaviours are deliberately not
...
- [x] 2.1 Write tests asserting the declaration rather than the node's name, in
      `tests/frontends.rs::declared_meanings`
- [x] 2.2 Declare Python's readings at the lowering sites as named constants beside `PY_TRUE_DIV`
      and its neighbours, each documenting the language it differs from

## 3. The Rust backend reproduces what the node declares

- [x] 3.1 Add the two mode enums to `runtime.rs`, self-contained, and take them as parameters on
      `py_index`, `PyIndexable::py_get`, and `PyLen::py_len`
- [x] 3.2 Emit the declared mode as a literal argument at the subscript and length call sites
- [x] 3.3 Add a test that the IR's mode enums and the emitted runtime's copies have the same
      variants, since they are two spellings of one decision and cannot be coupled directly
- [x] 3.4 Rewrite the runtime module doc's "not everything here is neutral yet" paragraph to name
...
- [x] 4.1 Both index origins, including a negative index, an index past either end, and the
      boundary at exactly the length
- [x] 4.2 All three text unit readings, against a two-byte character and a character outside the
      basic plane, so all three readings disagree
- [x] 4.3 `py_key`'s missing-key message, `PySetItem` inserting and overwriting, `PyContains` over
      sequence, mapping, set, and string, and `PyIterate` over all three containers
- [x] 4.4 Confirm no helper in `runtime.rs` is left without a test, and that its coverage clears
      ~90% from 57.95%

## 5. Execution tests for the readings Python cannot write

- [x] 5.1 Hand-built IR declaring `FromStart` and `Utf8Bytes`, compiled and run, in
      `tests/execution.rs::modes_python_cannot_write`
- [x] 5.2 A test that the same program under each reading produces different output, so a backend
