...
To achieve a truly universal Intermediate Representation (IR), we must minimize divergence when representing equivalent operations across different source languages (e.g., Python vs TypeScript). If language frontends diverge structurally on identical concepts, we lose the value of a shared middle-end and optimization pipeline. By strictly enforcing that backend-agnostic frontends emit equivalent structural shapes for identical operations (while explicitly preserving necessary semantic checking modes), we maximize shared paths and minimize the N x M cost of language scaling.

## What Changes

- Introduce a TypeScript Frontend (`typescript-frontend`) that lowers TS AST to `compylr` IR.
- Introduce a Go Backend (`go-backend`) that consumes `compylr` IR and emits Go code.
...
- Hook the Difference Checker into the CI and fixture corpus to iteratively track and minimize `D(I_python, I_ts)` without breaking execution correctness.

## Capabilities

### New Capabilities
- `typescript-frontend`: Parses a strictly annotated subset of TypeScript and lowers it to `compylr` IR.
- `go-backend`: Generates idiomatic Go code from `compylr` IR.
...
- **Codebase:** Adds two major language boundaries (`compylr-frontend-typescript` and `compylr-backend-go`).
