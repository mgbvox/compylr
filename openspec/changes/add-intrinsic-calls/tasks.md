...
- [ ] 1.1 Write the failing tests first: an intrinsic resolves without a matching unit member; a
      ...
      order.
- [ ] 1.2 Add the intrinsic signature registry to `compylr-ir`, keyed by module and operation,
      carrying parameter types, result type, and whether the operation is fallible.
...
- [ ] 1.4 Add the namespaced intrinsic expression form to `Expr`, carrying module, operation,
      arguments, and an optional checking mode.
- [ ] 1.5 Advance `ARTIFACT_VERSION` and confirm no reader for the previous version is kept.
- [ ] 1.6 Extend the fingerprint over the new form; assert two units differing only in the operation
      fingerprint differently.
- [ ] 1.7 Round-trip an intrinsic through the artifact and assert the recovered unit matches.
- [ ] 1.8 Confirm `cargo test -p compylr-ir` passes and `crate_boundaries.rs` still holds — the
      registry must not have pulled a dependency into `compylr-ir`.
...
- [ ] 2.2 Replace the blanket import rejection at `lower.rs:582` with resolution against the
      registry, keeping the rejection for unsupported modules and for from-imports.
...
- [ ] 3.6 Add the frontend spellings so a diagnostic quotes the module and operation the way the
      user wrote them, in `spelling.rs` and not in the IR.

## 4. Verification and folding

- [ ] 4.1 Make `Unit::validate` resolve intrinsics against the registry and leave `Expr::Call`
      resolution unchanged.
- [ ] 4.2 Confirm constant folding either folds an intrinsic over literal arguments or leaves it
      untouched — and that whichever it does, it preserves the checking mode.
...
- [ ] 5.2 Add the `math` emission table mapping each operation onto its `f64` method or
...
- [ ] 5.5 Extend `tests/conformance.rs` so an intrinsic is covered in every position it is legal in
      — free function body, method body, constructor body, and loop body.
...
- [ ] 6.1 Write the failing test: a program using `math` compiled for Go fails reporting the mapping
      as planned.
- [ ] 6.2 Add the refusal on the (module, backend) pair, distinct from the backend being unknown or
      unimplemented.
- [ ] 6.3 Confirm a Go compilation of a program using no module still succeeds unchanged.

## 7. Corpus

- [ ] 7.1 Add `frontends/python/fixtures/accepted/math_module.py` exercising every registry
      operation, with a unique member name across the whole accepted corpus.
- [ ] 7.2 Add its driver in `frontends/python/fixtures/drivers/`, naming calls as literal data and
      carrying no expected values.
- [ ] 7.3 Compare floating-point answers within a stated tolerance and non-finite answers by
      classification.
- [ ] 7.4 Add rejected fixtures: unsupported module, unsupported operation of a supported module,
      from-import, module as a value, and exponentiation still refused.
...
- [ ] 8.1 Confirm a checked domain failure crosses the bridge as an exception, not an abort.
- [ ] 8.2 Confirm the generated crate needs no new dependency for `math`.

## 9. Documentation and checks

- [ ] 9.1 Regenerate the README subset matrix with `scripts/update_subset.py`; confirm `--check`
      passes, and never hand-edit the block.
- [ ] 9.2 Update the README prose half: imports, the supported module list, and the module-is-not-a-
      value rule.
- [ ] 9.3 Update `CLAUDE.md`: the new artifact version, the intrinsic form and why it is not a call,
      the from-import refusal, and the Go reservation.
- [ ] 9.4 Run `cargo fmt --all`, `cargo clippy --workspace --all-targets -- -D warnings`, and
      `cargo test --workspace`.
- [ ] 9.5 Run `make check` and confirm CI, the Makefile, and the hooks all still agree.
- [ ] 9.6 Remove `.compylr` and `demo/.compylr` before any measurement, then run `make demo` and
      confirm no regression on programs that use no module.
