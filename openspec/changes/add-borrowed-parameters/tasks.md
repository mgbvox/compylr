...
- [ ] 0.2 Run [`a_text_parameter_is_usable_in_every_position`](../../../crates/compylr-host-python/tests/execution.rs#L2728)
      and record that it passes, as the before-state of the gate.
...
- [ ] 1.2 Add the passing mode — owned, shared borrow, mutable borrow — to `Param` in
      [`ir.rs`](../../../crates/compylr-ir/src/ir.rs#L103).
- [ ] 1.3 Make owned the `Default`, so a hand-built unit is conservative by construction — this is
      the property the reverted attempt did not have.
- [ ] 1.4 Advance [`ARTIFACT_VERSION`](../../../crates/compylr-ir/src/ir.rs#L58) and extend
      [`Unit::fingerprint`](../../../crates/compylr-ir/src/ir.rs#L1299) over the mode.
- [ ] 1.5 Confirm the mode carries no target spelling, and that
...
- [ ] 2.3 Extend the existing mutability fixpoint in
      [`lower.rs`](../../../crates/compylr-frontend-python/src/lower.rs) to decide ownership in the
      same pass; do not add a second analysis.
...
- [ ] 3.2 Emit signatures in [`rust.rs`](../../../crates/compylr-backend-rust/src/rust.rs) from the
      mode, never from the type or from whether the parameter is mutated.
- [ ] 3.3 Remove the clone on reading a borrowed parameter.
- [ ] 3.4 Build every fixture in [`accepted/`](../../../frontends/python/fixtures/accepted/) and
      confirm the generated crates compile — especially the four forcing shapes.
...
- [ ] 4.2 Convert each argument by its mode in
      [`compylr-bridge-python-rust`](../../../crates/compylr-bridge-python-rust/src/lib.rs),
      borrowing the host's buffer for text where possible.
...
- [ ] 5.2 Move the proposal's worked example into
      [`accepted/`](../../../frontends/python/fixtures/accepted/) as `ownership.py` with a driver in
      [`drivers/`](../../../frontends/python/fixtures/drivers/), with member names unique across the
      whole accepted corpus — `is_long` borrows, `roster` is owned, and the case asserts both modes.
- [ ] 5.3 Add a case per forcing shape asserting the parameter's mode is owned, inspecting the unit
      rather than emitted text.
- [ ] 5.4 Add the coverage check that fails when a shape's case asserts only the answer and not the
      mode.
- [ ] 5.5 Add the check that fails when a previously owned shape becomes borrowed, so such a change
      is deliberate.

## 6. Measurement

- [ ] 6.1 Remove `.compylr/` and `demo/demo-python-rust/.compylr/` before measuring anything.
- [ ] 6.2 Measure the per-element text conversion cost borrowed against owned, and record it.
- [ ] 6.3 Run `make demo` and confirm no algorithm regresses beyond noise.
...
- [ ] 7.1 Rewrite the [`CLAUDE.md`](../../../CLAUDE.md) note about the reverted `&str` work so it
      states the rule — a parameter is owned when it escapes — rather than warning against trying.
- [ ] 7.2 Update README prose covering how arguments cross, including that collections still convert
      element by element.
- [ ] 7.3 Regenerate the README benchmark tables with
